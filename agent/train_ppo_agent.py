...
class TrainPPOAgent(TrainAgent):
    ...
    def __init__(self, cfg):
        ...
        self.actor_optimizer = torch.optim.AdamW(
            self.model.actor_ft.parameters(),  # 仅优化微调部分
            lr=cfg.train.actor_lr,  # 典型值3e-4
            weight_decay=cfg.train.actor_weight_decay  # L2正则
        )
        # use cosine scheduler with linear warmup
        self.actor_lr_scheduler = CosineAnnealingWarmupRestarts(
            self.actor_optimizer,
            first_cycle_steps=cfg.train.actor_lr_scheduler.first_cycle_steps, # 周期长度
            cycle_mult=1.0,
            max_lr=cfg.train.actor_lr,
            min_lr=cfg.train.actor_lr_scheduler.min_lr,
            warmup_steps=cfg.train.actor_lr_scheduler.warmup_steps, # 线性预热步数
            gamma=1.0,
        )
        ...
        self.vf_coef: float = cfg.train.get("vf_coef", 0.05)
        ...
        self.bc_loss_coeff: float = cfg.train.get("bc_loss_coeff", 0.01)
