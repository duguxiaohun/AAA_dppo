...
class Transformer(nn.Module):
    def __init__(
        self,
        output_dim,        # 输出特征维度，如动作均值、logvar等
        horizon,           # 解码长度（生成多少步动作）
        cond_dim,          # 条件输入维度（例如状态/观测）
        T_cond=1,          # 条件观测的时间长度，通常为1
        n_layer=12,        # decoder层数
        n_head=12,         # 多头注意力的头数
        n_emb=768,         # embedding维度
        p_drop_emb=0.0,    # embedding dropout概率
        p_drop_attn=0.0,   # 注意力 dropout 概率
        causal_attn=False, # 是否使用因果mask（左看右屏蔽）
        n_cond_layers=0,   # encoder的层数，0表示非transformer结构
    ...
    def forward(
        ...
        **kwargs,
        ...
        print(cond.shape)
        """
        cond: (B, T, cond_dim)   torch.Size([4, 1, 16])
        output: (B, T, output_dim)
        """
        # encoder
        cond_embeddings = self.cond_obs_emb(cond)  # (B,T,n_emb)
        tc = cond_embeddings.shape[1] #T
        ...
        # print(self.mask, self.memory_mask)
        # torch.Size([B, horizon, n_emb])
...
if __name__ == "__main__":
    transformer = Transformer(
        ...
        T_cond=1,
        cond_dim=16,
        causal_attn=True,  # no need to use for delta control
