...
OVERVIEW_CAMERA_HEIGHT = 95.0
OVERVIEW_CAMERA_PITCH = -90.0
OVERVIEW_CAMERA_LOCATION = (-51.0, -43.0)
OVERVIEW_CAMERA_YAW = 180.0
...
class InterSection(gym.Env):
    ...
    def get_actor_state_total(self, actor, types):
        ...
        steering = actor.get_control().steer  # 获取轮胎转角

        # 返回车辆的状态：位置 (x, y), 速度 (vx, vy), 朝向 (yaw), 加速度 (ax, ay), 轮胎转角
        return [location.x, location.y,
                velocity.x, velocity.y,
                acceleration.x, acceleration.y,
                steering,
                self.angle_norm(actor.get_transform().rotation.yaw)]  # 朝向（以角度表示）
            # return {
            #     'location': [location.x, location.y],
            #     'velocity': [velocity.x, velocity.y],
            #     'acceleration': [acceleration.x, acceleration.y],
            #     'steering': steering,
            #     'yaw': self.angle_norm(actor.get_transform().rotation.yaw)  # 朝向（以角度表示）
            # }
    ...
    def spect_cam(self, vehicle):
        ...
        center_x, center_y = OVERVIEW_CAMERA_LOCATION
        ...
        cam_loc = carla.Location(
            x=center_x,
            y=center_y,
            z=z + OVERVIEW_CAMERA_HEIGHT,
        )
        cam_rot = carla.Rotation(
            pitch=OVERVIEW_CAMERA_PITCH,
            yaw=OVERVIEW_CAMERA_YAW,
            roll=0.0,
        )
