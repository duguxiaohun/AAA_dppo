...
class InterSection(gym.Env):
    ...
    def reset(self):
        ...
        # spawn_point_ego.location.x = -20 + 20 * random.random()
        ...
        self.traffic_manager.vehicle_percentage_speed_difference(self.ego_vehicle,
                                                                 self.ego_current_speed_ratio)  # 车辆的预期速度与其当前道路速度限制的差值
    ...
    def get_all_waypoints(self, vehicle, judge=False):
        ...
        # [(-1.0804308652877808, -64.49608612060547), (-1.5807839632034302, -64.50405883789062),
        #  (-2.082475423812866, -64.51168060302734), (-2.584174633026123, -64.51884460449219),
        #  (-3.085880756378174, -64.52545928955078), (-3.5875933170318604, -64.5316162109375),
        #  (-4.089311599731445, -64.5372543334961), (-4.589837074279785, -64.54248809814453),
        #  (-5.089810371398926, -64.54769897460938), (-5.5904412269592285, -64.55290985107422),
        #  (-6.090412616729736, -64.55812072753906), (-6.590385913848877, -64.56332397460938),
        #  (-7.090359210968018, -64.56853485107422), (-7.590332508087158, -64.57374572753906),
        #  (-8.090306282043457, -64.57894897460938), (-8.590277671813965, -64.58415985107422),
        #  (-9.090250968933105, -64.58936309814453), (-9.590224266052246, -64.59457397460938),
        #  (-10.090197563171387, -64.59977722167969), (-10.590170860290527, -64.60498809814453),]
    ...
    def step(self, action):
        ...
        # speed_reward = -0.02 * np.abs(velocity_ego -1.5) if velocity_ego < 1.5 else 0
        ...
        if step_dist < 0.1:
            self.stop_step += 1
            if self.stop_step > 10:

                if not self.stop_once and x_ego < -36:
                    self.stop_once = True
                    self.control.throttle = 0.5
                    print('force')
                elif self.stop_once and x_ego < -36 and self.random > 0.8:
                    self.stop_twice = True
        ...
        if self.stop_twice:
            self.control.throttle = 0.5
            print('Forceing moving forward due to being stuck')
        ...
        # speed_reward = 1 - np.abs(
        #     velocity_ego - self.desire_speed) / self.desire_speed if velocity_ego < self.desire_speed else 1
        ...
        if done:
            self.destroy()
            print(self.add_speed, self.add_speed / self.count)
