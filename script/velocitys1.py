...
MAX_PLOT_STEP = 180 # 【新增】最大时间步，超过此长度不画
VLINE_X = 110      # 竖线位置

ep_name = f"{ep_id:05d}"
names = ["ego", "v_2", "v_3", "v_5"]  # 按需增减
...
data_dict = {}
max_step = 0
print(f"Reading Episode {ep_name}, Trimming first {TRIM_START} steps, Max duration {MAX_PLOT_STEP}...")
        ...
        for name in names:
            if name in grp:
                ...
                if len(speeds_arr) > TRIM_START:
                    ...
                    # 【新增】在此处限制最大长度为 150
                    if MAX_PLOT_STEP is not None:
                        trimmed_speeds = trimmed_speeds[:MAX_PLOT_STEP]
...
# ====== 7. 添加美化的竖线 (t=110) ======
# 注意：如果 MAX_PLOT_STEP 设置小于 110，max_step 也会变小，竖线就不会画出来，逻辑正确
if VLINE_X < max_step:
...
# ====== 9. 保存图片 ======
png_filename = f"Velocity_Ep{ep_name}.png"
pdf_filename = f"Velocity_Ep{ep_name}.pdf"
