...
class TrainAgent:
    ...
    def __init__(self, cfg):
        ...
        self.reward_scale = cfg.env.reward_scale
        self.tau = cfg.tau
    ...
    def load(self, checkpoint_dir, itr):
        """
        loads model from disk
        """
        loadpath = os.path.join(checkpoint_dir, f"state_{itr}.pt")
        data = torch.load(loadpath, weights_only=True)

        self.itr = data["itr"]
        self.model.load_state_dict(data["model"])
        print('Succcess load model!')
    ...
    def reset_env_all(self, verbose=False, options_venv=None, **kwargs):
        ...
        # ---- 仅处理三元组观测：neighbor_trajs / ego_state / neighbor_waypoints ----
        import numpy as np
        ...
        if isinstance(obs_venv, list) and len(obs_venv) > 0:
            ...
            if isinstance(first, tuple) and len(first) == 3:
                neighbor_trajs = np.stack([np.asarray(x[0]) for x in obs_venv], axis=0)
                ego_state = np.stack([np.asarray(x[1]) for x in obs_venv], axis=0)
                neighbor_wps = np.stack([np.asarray(x[2]) for x in obs_venv], axis=0)
                obs_venv = {
        ...
        elif isinstance(obs_venv, tuple) and len(obs_venv) == 3:
            # 单环境 3 元组 → 加 batch 维
            neighbor_trajs = np.asarray(obs_venv[0])[None, ...]
            ego_state = np.asarray(obs_venv[1])[None, ...]
            neighbor_wps = np.asarray(obs_venv[2])[None, ...]
            obs_venv = {
                "neighbor_trajs": neighbor_trajs,
                "ego_state": ego_state,
                "neighbor_waypoints": neighbor_wps,
        ...
        elif isinstance(obs_venv, dict):
            # 若上游已是 dict，则仅保留并转为 ndarray（其它键丢弃）
            keep = ("neighbor_trajs", "ego_state", "neighbor_waypoints")
            obs_venv = {k: np.asarray(obs_venv[k]) for k in keep if k in obs_venv}
            missing = [k for k in keep if k not in obs_venv]
            if missing:
                raise KeyError(f"Missing keys in obs_venv dict: {missing}")

        else:
            raise TypeError(f"Unsupported obs_venv type for 3-tuple handling: {type(obs_venv)}")


        # print(obs_venv["neighbor_trajs"].shape, obs_venv["ego_state"].shape, obs_venv["neighbor_waypoints"].shape)
